class Aether::UserPowerful < ApplicationRecord
  belongs_to :user
  belongs_to :powerful, class_name: 'Aether::Powerful'
  ...
  def sort_priority
    ...
    if self.priority.nil?
      ...
      others.select { |other| other >= self.priority }.each do |num|
        Aether::UserPowerful.find_by(user: self.user, priority: num).update(priority: priority + 1)
      end
    end
  end
end
